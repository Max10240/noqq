...
const download = (options, callback) => {
    ...
    const filePath = options.filename ? path.join(folder, options.filename) : path.join(folder, options.path.toString(), filename.split(/[?#]/)[0])
    console.log("fuck get path from dm:", filename, options.filename, filePath, options)
